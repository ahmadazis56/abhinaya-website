import React from 'react';
import Link from 'next/link';
import Image from 'next/image';

const footerData = {
  company: {
    name: "ABHINAYA INDO GROUP",
    tagline: "Transforming Ideas into Digital Excellence",
    description: "Leading provider of innovative IT solutions, creative branding, and scientific publishing services across Indonesia and globally.",
    contact: {
      email: "info@abhinaya.co.id",
      phone: "+62 856-4660-3602",
      address: "Lombok, Indonesia"
    }
  },
  divisions: [
    {
      title: "Abhinaya Techno",
      description: "Cutting-edge IT solutions",
      icon: "💻",
      link: "/abhinaya-techno"
    },
    {
      title: "Abhinaya Creative", 
      description: "Creative branding & design",
      icon: "🎨",
      link: "/abhinaya-creative"
    },
    {
      title: "Abhinaya Publisher",
      description: "Scientific publishing",
      icon: "📚",
      link: "/abhinaya-publisher"
    }
  ],
  quickLinks: [
    { title: "About", links: [
      { label: "Company Profile", href: "/about" },
      { label: "Our Team", href: "/team" },
      { label: "Contact", href: "/contact" }
    ]},
    { title: "Services", links: [
      { label: "Web Development", href: "/abhinaya-techno" },
      { label: "Mobile Apps", href: "/abhinaya-techno" },
      { label: "Branding", href: "/abhinaya-creative" },
      { label: "Publishing", href: "/abhinaya-publisher" }
    ]},
    { title: "Resources", links: [
      { label: "Portfolio", href: "/portfolio" },
      { label: "Blog", href: "https://news.abhinaya.co.id/" }
    ]}
  ],
  legal: [
    { label: "Privacy Policy", href: "/privacy" },
    { label: "Terms of Service", href: "/terms" },
    { label: "Cookie Policy", href: "/cookies" }
  ],
  social: [
    { name: "Facebook", icon: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z", href: "#" },
    { name: "Twitter", icon: "M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z", href: "#" },
    { name: "LinkedIn", icon: "M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z", href: "#" },
    { name: "Instagram", icon: "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.645.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.014-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zM5.838 12a6.162 6.162 0 1112.324 0 6.162 6.162 0 01-12.324 0zM12 16a4 4 0 110-8 4 4 0 010 8zm4.965-10.405a1.44 1.44 0 112.881.001 1.44 1.44 0 01-2.881-.001z", href: "#" }
  ]
};

export default function UltraProfessionalFooter() {
  return (
    <footer className="bg-gradient-to-r from-[#0e6d7c] to-[#14aecf] border-t border-[#0e6d7c]">
      {/* Main Footer */}
      <div className="w-full px-6 sm:px-8 lg:px-12 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          
          {/* Brand Column - 3 columns */}
          <div className="lg:col-span-3">
            <div className="mb-8">
              <div className="flex items-center space-x-4 mb-6">
                <div className="relative w-24 h-24">
                  <Image
                    src="/images/logo.png"
                    alt="Abhinaya Indo Group Logo"
                    fill
                    className="object-contain"
                  />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">ABHINAYA</h3>
                  <p className="text-xs text-white/80 font-medium">INDO GROUP</p>
                </div>
              </div>
              <p className="text-base font-semibold text-white mb-4">
                {footerData.company.tagline}
              </p>
              <p className="text-white/80 leading-relaxed text-sm">
                {footerData.company.description}
              </p>
            </div>
          </div>


          {/* Quick Links Columns - 9 columns */}
          <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-3 gap-8">
            {footerData.quickLinks.map((section, index) => (
              <div key={index}>
                <h3 className="text-lg font-bold text-white mb-6">{section.title}</h3>
                <ul className="space-y-3">
                  {section.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      <Link
                        href={link.href}
                        className="text-white/80 hover:text-white transition-colors duration-200 flex items-center group"
                      >
                        <span>{link.label}</span>
                        <svg className="w-4 h-4 ml-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                        </svg>
                      </Link>
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </div>
      </div>


      {/* Bottom Bar */}
      <div className="bg-[#0a4f5a]">
        <div className="w-full px-6 sm:px-8 lg:px-12 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="text-white/60 text-sm mb-4 md:mb-0">
              © {new Date().getFullYear()} {footerData.company.name}. All rights reserved.
            </div>
            <div className="flex items-center space-x-6">
              {footerData.legal.map((link, index) => (
                <Link
                  key={index}
                  href={link.href}
                  className="text-white/60 hover:text-white text-sm transition-colors duration-200"
                >
                  {link.label}
                </Link>
              ))}
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
